...
class Usuario {
  constructor(id, nombre, email, password, isAdmin = true, isLogged = false) {
    this.id = id;
    this.nombre = nombre;
    this.email = email;
...
formLogin.addEventListener("submit", (e) => {
  ...
  sessionStorage.setItem("usuarioLogueado", JSON.stringify(validarEmail));
  formLogin.reset();
  Swal.fire({
    position: "center",
    icon: "success",
  ...
  //redireccionar al usuario a la pantalla de login
  //   window.location.href = "../html/hola.html";
  setTimeout(() => {
    window.location.href = "../html/hola.html";
  }, 2000);
});
...
// si es admin lo mando a una pagina, si es usuario lo mando a otra

// si guardo algo en session storage cuando cierro el navegador se desloguea

// cambiarRol con un modal que se abra y pregunte a cual ID desea cambiar el rol. Si existe, cambiar a que rol y realizar el cambio
